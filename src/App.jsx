...
function App() {
  ...
  const importaDaExcel = async (e) => {
    ...
    try {
      // Carica la libreria XLSX dinamicamente
      ...
      if (firebaseReady && db) {
        ...
        // Verifica titoli esistenti per evitare duplicati
        const snapshot = await firestore.getDocs(firestore.collection(db, "portafoglio"));
        const titoliEsistenti = snapshot.docs.map(d => ({
          id: d.id,
          nome: d.data().nome,
        }));

        for (const row of jsonData) {
          const duplicato = titoliEsistenti.find(t => t.nome === row.nome);

          const dataToInsert = {
            nome: row.nome || "",
            prezzo_acquisto: parseFloat(row.prezzo_acquisto) || 0,
            prezzo_corrente: parseFloat(row.prezzo_corrente) || 0,
            tipologia: row.tipologia || "",
            dividendi: parseFloat(row.dividendi) || 0,
            prelevato: parseFloat(row.prelevato) || 0,
            percentuale_12_mesi: parseFloat(row.percentuale_12_mesi) || 0,
            rendimento_percentuale: parseFloat(row.rendimento_percentuale) || 0,
            payback: parseFloat(row.payback) || 0,
            score: parseFloat(row.score) || 0,
          };

          if (duplicato) {
            const sovrascrivere = window.confirm(
              `Il titolo "${row.nome}" esiste già. Sovrascrivere?`
            );
            if (sovrascrivere) {
              await firestore.updateDoc(firestore.doc(db, "portafoglio", duplicato.id), dataToInsert);
            }
          } else {
            await firestore.addDoc(firestore.collection(db, "portafoglio"), dataToInsert);
          }
        }
      ...
      alert("Importazione Excel completata!");
    } catch (error) {
      console.error("Errore importazione Excel:", error);
      alert("Errore durante l'importazione. Verifica che il file Excel sia valido.");
    }
  };
  ...
  const esportaExcel = async () => {
    try {
      ...
      const dataForExport = dati.map(item => ({
        nome: item.nome,
        prezzo_acquisto: item.prezzo_acquisto,
        prezzo_corrente: item.prezzo_corrente,
        tipologia: item.tipologia,
        dividendi: item.dividendi,
        prelevato: item.prelevato,
        percentuale_12_mesi: item.percentuale_12_mesi,
        rendimento_percentuale: item.rendimento_percentuale,
        payback: item.payback,
        score: item.score,
        profitto: item.prezzo_corrente - item.prezzo_acquisto + item.dividendi + item.prelevato
      }));
